...
\date{January 2026\\[0.5em]\small{Paper version 1.4 · Schema version 1.4}}
...
\begin{itemize}
    ...
    \item \textbf{tests}: Each of the five tests with pass/fail result, quantitative metrics, and evidence arrays
\end{itemize}
...
\begin{itemize}
    \item \textbf{v1.0}: Initial schema with single manifest structure
    \item \textbf{v1.2}: Refactored document fields to arrays for population-scale systems
    \item \textbf{v1.3}: Added named object arrays (repositories, escalation\_paths, learned\_components)
    \item \textbf{v1.4}: Two-manifest architecture (infrastructure.json + corrigibility.json), added operations section with SLOs, made assessed\_at required, added evidence arrays to all tests
\end{itemize}
